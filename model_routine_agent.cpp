...
void ModelRoutine::addSpAgents( const BOOL init, const VIdx& startVIdx, const VIdx& regionVSize, const IfGridBoxData<BOOL>& ifGridHabitableBoxData, Vector<VIdx>& v_spAgentVIdx, Vector<SpAgentState>& v_spAgentState, Vector<VReal>& v_spAgentVOffset ) {/* initialization */
   ...
   if( init == true ) {
      ...
      for( S64 j = 0 ; j < numCells; j++ ) {
         ...
         cellrad = A_MIN_CELL_RADIUS[AGENT_CELL_A] + (A_CELL_RADIUS[AGENT_CELL_A] - A_MIN_CELL_RADIUS[AGENT_CELL_A])*randScale;
         biomass = volume_agent( cellrad )*A_DENSITY_BIOMASS[ AGENT_CELL_A ] ;
...
void ModelRoutine::updateSpAgentState( const VIdx& vIdx, const JunctionData& junctionData, const VReal& vOffset, const NbrUBEnv& nbrUBEnv, SpAgentState& state/* INOUT */ ) {
    ...
    if ( type == AGENT_CELL_A )  {
        ...
        if ( A_AGENT_BOND_BOUNDARY_S[mtype] > 0.0 ) {
           ...
           REAL z = ( (REAL)vIdx[2] + 0.5 )*IF_GRID_SPACING + vOffset[2];
           REAL dist_b = FABS( z );
           if ( state.getModelInt(CELL_MODEL_INT_BOND_B) == 0 ) {
              if (dist_b < R0*A_AGENT_BOND_BOUNDARY_CREATE[mtype]){
                 state.setModelInt(CELL_MODEL_INT_BOND_B,1);
              }
           }
           else{
              if (dist_b> R0*A_AGENT_BOND_BOUNDARY_DESTROY[mtype]){
...
void ModelRoutine::adjustSpAgent( const VIdx& vIdx, const JunctionData& junctionData, const VReal& vOffset, const MechIntrctData& mechIntrctData, const NbrUBEnv& nbrUBEnv, SpAgentState& state/* INOUT */, VReal& vDisp ) {/* if not dividing or disappearing */
 ...
 // }
  ...
  if (state.getModelInt(CELL_MODEL_INT_BOND_B) == 1) {
        ...
        mag = xij * tanh( FABS(xij) *  A_AGENT_BOND_S[mtype][0]  );
        stress += A_AGENT_STIFFNESS[NUM_MECH_TYPES][NUM_MECH_TYPES] * dist_b * mag ; // stress
        vForce[2] +=  A_AGENT_STIFFNESS[NUM_MECH_TYPES][NUM_MECH_TYPES] * mag ;
  }
